...
void MemIndex_AddPostingList(MemIndex *index, char *word, DocID_t docid,
  ...
  if (!HashTable_Find(index, key, &mi_kv)) {
    ...
    Verify333(wp != NULL);
    wp->word = word;
    wp->postings = HashTable_Allocate(10);

    postings_kv.key = key;
    postings_kv.value = wp;

    Verify333(!HashTable_Insert(index, postings_kv, &unused));
...
LinkedList* MemIndex_Search(MemIndex *index, char *query[], int querylen) {
  ...
  while (HTIterator_IsValid(it)) {
    ...
    result->docid = (DocID_t) kv.key;
    result->rank = LinkedList_NumElements((LinkedList*) kv.value);

    LinkedList_Append(retlist, result);
    HTIterator_Next(it);
  }
  HTIterator_Free(it);
  ...
  for (i = 1; i < querylen; i++) {
    ...
    for (j = 0; j < numdocs; j++) {
      ...
      if (HashTable_Find(wp->postings, result->docid, &kv)) {
